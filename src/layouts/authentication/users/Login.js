...
const Login = () => {
  ...
  const handleLogin = (e) => {
    ...
    const hardcodedUser = {
      email: 'crunchquest@gmail.com',
      password: 'password123'
    };

    if (loginUser.email === hardcodedUser.email && loginUser.password === hardcodedUser.password) {
      // Simulating successful login
      setTimeout(() => {
        dispatchAuth({ type: 'LOGIN', payload: 'user-uid' });
        dispatchAuthRole({ type: 'LOGIN_ROLE', payload: 'admin' });
